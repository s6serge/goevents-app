  ...
  return (
    <header className="bg-[#1E3A8A] text-white py-6 md:py-8 relative z-50">
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        {/* Navigation Links */}
        <ul
          className={`absolute top-full right-4 bg-[#1E3A8A] shadow-lg w-[75vw] max-w-[350px] rounded-lg flex flex-col gap-3 py-4 px-4 transition-transform duration-300 ease-in-out ${menuOpen ? "block" : "hidden"} md:static md:w-auto md:max-w-none md:shadow-none md:bg-transparent md:flex md:flex-row md:gap-8 md:py-0 md:px-0`}
        >
          {navLinks.map((link) => (
            <li key={link.label}>
              <a
                ...
                onClick={() => setPage(link.label.toLowerCase())} // Handle page change
              >
                {link.label}
              </a>
            </li>
